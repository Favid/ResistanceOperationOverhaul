class UITactical_PromotionHero extends UIScreen;

const NUM_ABILITIES_PER_COLUMN = 4;

var array<UITactical_PromotionHeroColumn> Columns;
...
// Added by me
var XComGameState_Unit StartingUnit;
var int PendingRank, PendingBranch;
var UISoldierHeader Header;

simulated function InitPromotion(XComGameState_Unit Soldier, XComGameState CheckGameState)
{
	local UITactical_PromotionHeroColumn Column;

	StartingUnit = Soldier;
	
	Column = Spawn(class'UITactical_PromotionHeroColumn', self);
	...
	Header = Spawn(class'UISoldierHeader', self).InitSoldierHeader(Soldier.GetReference(), CheckGameState);
...
simulated function PopulateData()
{
	...
	HeaderString = class'UIArmory_PromotionHero'.default.m_strAbilityHeader;
	if (Unit.GetRank() != 1 && Unit.HasAvailablePerksToAssign())
	{
		HeaderString = class'UIArmory_PromotionHero'.default.m_strSelectAbility;
	}
	...
	//AS_SetHeaderData(Caps(FactionState.GetFactionTitle()), Caps(Unit.GetName(eNameType_FullNick)), HeaderString, class'UIArmory_PromotionHero'.default.m_strSharedAPLabel, class'UIArmory_PromotionHero'.default.m_strSoldierAPLabel);
	//AS_SetAPData(GetSharedAbilityPoints(), Unit.AbilityPoints);
	//AS_SetCombatIntelData(Unit.GetCombatIntelligenceLabel());
	//AS_SetPathLabels(class'UIArmory_PromotionHero'.default.m_strBranchesLabel, ClassTemplate.AbilityTreeTitles[0], ClassTemplate.AbilityTreeTitles[1], ClassTemplate.AbilityTreeTitles[2], ClassTemplate.AbilityTreeTitles[3]);
	...
	for (iRank = 0; iRank < maxRank; ++iRank) // Issue #1 -- new maxRank needs to be included
	{
		...
		// Start Issue #408
		Column.AS_SetData(bHighlightColumn, class'UIArmory_PromotionHero'.default.m_strNewRank, Unit.GetSoldierRankIcon(iRank+1), Caps(Unit.GetSoldierRankName(iRank+1)));
		// End Issue #408
	}
...
function bool UpdateAbilityIcons(out UITactical_PromotionHeroColumn Column)
{
	...
	for (iAbility = 0; iAbility < NUM_ABILITIES_PER_COLUMN; iAbility++)
	{
		...
		if (AbilityTemplate != none)
		{
			...
			{
				AbilityName = class'UIUtilities_Text'.static.GetColoredText(class'UIArmory_PromotionHero'.default.m_strAbilityLockedTitle, eUIState_Disabled);
...
function PreviewAbility(int Rank, int Branch)
{
	...
	CostLabel = class'UIArmory_PromotionHero'.default.m_strCostLabel;
	APLabel = class'UIArmory_PromotionHero'.default.m_strAPLabel;
	...
	if (Rank >= Unit.GetRank())
	{
		...
		AbilityName = class'UIUtilities_Text'.static.GetColoredText(class'UIArmory_PromotionHero'.default.m_strAbilityLockedTitle, eUIState_Disabled);
		AbilityDesc = class'UIUtilities_Text'.static.GetColoredText(class'UIArmory_PromotionHero'.default.m_strAbilityLockedDescription, eUIState_Disabled);
	...
	else
	{
		...
		if (AbilityTemplate != none)
		{
			...
			else if (AbilityTemplate.PrerequisiteAbilities.Length > 0)
			{
				...
				if (MutuallyExclusiveNames != "")
				{
					AbilityDesc = class'UIUtilities_Text'.static.GetColoredText(class'UIArmory_PromotionHero'.default.m_strMutuallyExclusiveAbility @ MutuallyExclusiveNames, eUIState_Warning) $ "\n" $ AbilityDesc;
				}

				if (PrereqAbilityNames != "")
				{
					AbilityDesc = class'UIUtilities_Text'.static.GetColoredText(class'UIArmory_PromotionHero'.default.m_strPrereqAbility @ PrereqAbilityNames, eUIState_Warning) $ "\n" $ AbilityDesc;
				}
				// End Issue #128
			}
		}
...
simulated function ConfirmAbilitySelection(int Rank, int Branch)
{
	...
	DialogData.strTitle = class'UIArmory_PromotionHero'.default.m_strConfirmAbilityTitle;
	...
	ConfirmAbilityText = `XEXPAND.ExpandString(class'UIArmory_PromotionHero'.default.m_strConfirmAbilityText);
	...
	if (AbilityPointCost > GetUnit().AbilityPoints)
	{
		...
		if((AbilityPointCost - GetUnit().AbilityPoints) == 1)
			ConfirmAbilityText $= "\n\n" $ `XEXPAND.ExpandString(class'UIArmory_PromotionHero'.default.m_strSharedAPWarningSingular);
		else
			ConfirmAbilityText $= "\n\n" $ `XEXPAND.ExpandString(class'UIArmory_PromotionHero'.default.m_strSharedAPWarning);
